<x-layout>
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="fw-bold">Edit Teacher Attendance</h4>
        ...
        {{-- Filter Form --}}
        <form method="GET" action="{{ route('teacher_attendance.edit') }}" class="row g-3 mb-3">
            <div class="col-md-4">
        ...
        @if(request('batch_id') && request('attendance_date'))
            <form action="{{ route('teacher_attendance.update') }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="card shadow-sm">
                    <div class="card-body">
                                ...
                                <tbody>
                                    @foreach($attendanceRecords as $record)
                                        <tr>
                                            ...
                                            <td>
                                                <input type="radio" name="attendance[{{ $record->teacher_id }}]" value="1"
                                                       {{ $record->is_present ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                                <input type="radio" name="attendance[{{ $record->teacher_id }}]" value="0"
                                                       {{ !$record->is_present ? 'checked' : '' }}>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        @else
                            <p class="text-muted mb-0">No teachers found for this batch.</p>
                        @endif
                    </div>
                </div>
