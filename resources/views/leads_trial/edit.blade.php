<x-layout>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold mb-0">✏️ Edit Lead Trial</h4>
        <a href="{{ route('leads_trial.index') }}" class="btn btn-secondary">← Back</a>
    </div>

    <div class="card shadow-sm border-0">
        <div class="card-body">
            <form action="{{ route('leads_trial.update', $trial->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" 
                        value="{{ $trial->lead->name }}" disabled>
                </div>

                {{-- Trial Start Date --}}
                <div class="mb-3">
                    <label class="form-label fw-semibold">Trial Start Date *</label>
                    <input type="date" name="trl_start_dt" id="startDate" class="form-control"
                        value="{{ old('trl_start_dt', $trial->trl_start_dt) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" id="endDate" class="form-control"
                        value="{{ $trial->trl_end_dt }}" readonly>
                </div>
                    ...
                    <select name="course_id" class="form-select" required>
                        ...
                        @foreach ($courses as $course)
                            <option value="{{ $course->id }}" 
                                {{ old('course_id', $trial->course_id) == $course->id ? 'selected' : '' }}>
                                {{ $course->title }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="batch_id" class="form-select" required>
                        ...
                        @foreach ($batches as $batch)
                            <option value="{{ $batch->id }}" 
                                {{ old('batch_id', $trial->batch_id) == $batch->id ? 'selected' : '' }}>
                                {{ $batch->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="comments" class="form-control" rows="3" required>{{ old('comments', $trial->comments) }}</textarea>
                    @error('comments') <small class="text-danger">{{ $message }}</small> @enderror
                </div>

                <div class="text-end">
                    <button class="btn btn-primary px-4">💾 Update Trial</button>
                </div>
            </form>
        </div>
    </div>
